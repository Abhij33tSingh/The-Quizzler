...
const QuizCreated = () => {
  ...
  useEffect(() => {
    ...
    Axios.get("http://localhost:3001/getUser/" + quiz.userId).then(
      (response) => {
        setUser(response.data.name);
      }
      //WE ARE GETTING ALL OF OUR USER DATA IN response.data
    );
  }, [params.id, quiz.userId]);
    ...
    <div>
      <header>
        ...
        <div className="ProfilenavbarItems">
          ...
          <nav onClick={() => navigate("/AboutUs/" + quiz.userId)}>
            About us
          </nav>
          <nav onClick={() => navigate("/Profile/" + quiz.userId)}>{user}</nav>
        </div>
      </header>
      <div>
